    ...
    async def query_database(query: str) -> str:
        try:
            mydb = mysql.connector.connect(
                ...
                database=os.getenv("DATABASE_NAME", "company_db")  # Updated to your database
            )
            mycursor = mydb.cursor()
            mycursor.execute(query)
            result = mycursor.fetchone()
            if result:
                return f"According to the database, {result[0]}"
            else:
                return "Sorry, I couldn't find any matching results in the database."
        except mysql.connector.Error as err:
            logger.error(f"Error querying the database: {err}")
            return "Sorry, there was an error querying the database."

    @session.on("response_created")
    async def on_response_created(response: openai.realtime.RealtimeResponse):
        # Check if the user's input contains a query about employees in a specific city
        if "employees in" in response.content.lower():
            city = response.content.lower().split("employees in")[1].strip()
            query = f"SELECT COUNT(*) FROM employee WHERE city = '{city}'"
            result = await query_database(query)
            response.content = result
            session.response.update(response)

        # Check if the user's input asks for a list of employees in a specific city
        elif "list employees in" in response.content.lower():
            city = response.content.lower().split("list employees in")[1].strip()
            query = f"SELECT name FROM employee WHERE city = '{city}'"
            result = await query_database(query)
            response.content = result
            session.response.update(response)
